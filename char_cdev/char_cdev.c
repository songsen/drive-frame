...
static ssize_t char_cdev_read(struct file *file, char *buf, size_t count,
			      loff_t *f_pos)
{
...
static ssize_t char_cdev_write(struct file *file, const char *buf, size_t count,
			       loff_t *f_pos)
{
...
static int char_cdev_ioctl(struct inode *inode, struct file *file,
...
static long char_cdev_unlocked_ioctl(struct file *f, unsigned int i, unsigned long l)
{
	printk(KERN_INFO DEVICE_NAME " ioctl method!\n");
	return 0;
}

struct file_operations char_cdev_fops = { .owner = THIS_MODULE,
					  .read = char_cdev_read,
					  .write = char_cdev_write,
					  .open = char_cdev_open,
					  .release = char_cdev_release,
					  .unlocked_ioctl = char_cdev_unlocked_ioctl
...
static int __init char_cdev_init(void)
{
	...
	if (major > 0) { /* 静态设备号 */
		...
		ret = alloc_chrdev_region(&devno, minor, 1,
					  "char_cdev"); /* 从系统获取主设备号 */
		major = MAJOR(devno);
	}
	if (ret < 0) {
	...
	if (char_cdev != NULL) {
		cdev_init(char_cdev,
			  &char_cdev_fops); /* 初始化 char_cdev 结构 */
		char_cdev->owner = THIS_MODULE;
		if (cdev_add(char_cdev, devno, 1) !=
		    0) { /* 增加 char_cdev 到系统中 */
