...
static char dev_rw_buff[64] = { "hello world!" }; /* 设备内部读写缓冲区 */
...
static ssize_t char_cdev_rw_read(struct file *file, char *buf, size_t count,
				 loff_t *f_pos)
{
	printk("count:%ld %s\n", count, dev_rw_buff);
...
static ssize_t char_cdev_rw_write(struct file *file, const char *buf,
...
static int __init char_cdev_rw_init(void)
{
	...
	if (char_cdev_rw != NULL) {
		cdev_init(char_cdev_rw,
			  &char_cdev_rw_fops); /* 初始化 char_cdev_rw 结构 */
		char_cdev_rw->owner = THIS_MODULE;
		if (cdev_add(char_cdev_rw, devno, 1) !=
		    0) { /* 增加 char_cdev_rw 到系统中 */
