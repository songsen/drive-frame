...
void gpio_direction_output(int io, int n)
{
	printk("led_drv: io=%d, n=%d\n", io, n);
}

void gpio_set_value(int io, int n)
{
	printk("led_drv: io=%d, n=%d\n", io, n);
}
...
static int __devinit led_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "led_drv: probe name: %s\n", pdev->name);

	res_io = platform_get_resource(pdev, IORESOURCE_IO,
				       0); /* 从设备资源获取 IO 引脚 */
	led_io = res_io->start;

	ret = alloc_chrdev_region(&devno, minor, 1,
				  DEVICE_NAME); /* 从系统获取主设备号 */
	major = MAJOR(devno);
	if (ret < 0) {
